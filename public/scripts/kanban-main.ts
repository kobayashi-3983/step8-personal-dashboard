  ...
  document.querySelectorAll('.column').forEach((col) => {
    ...
    tasks
      ...
      .forEach((task) => {
        ...
        // --- チェック項目追加 ---
        el.querySelector('.add-check-btn')?.addEventListener(
          'click',
          async () => {
        ...
        el.querySelectorAll('.check-item').forEach((cb) => {
          cb.addEventListener('change', async (e) => {
            const idx = Number((e.target as HTMLInputElement).dataset.id);
            checklist[idx].checked = (e.target as HTMLInputElement).checked;
        ...
        el.querySelectorAll('.del-check').forEach((btn) => {
          btn.addEventListener('click', async (e) => {
            const idx = Number((e.target as HTMLElement).dataset.id);
        ...
        // --- タスク削除 ---
        el.querySelector('.delete-task')?.addEventListener(
          'click',
          async () => {
...
document.querySelectorAll('.add-task-btn').forEach((btn) => {
  btn.addEventListener('click', async () => {
    const status = (btn.parentElement as HTMLElement).dataset.status!;
...
function enableDrag() {
  ...
  document.querySelectorAll('.task').forEach((t) => {
    t.addEventListener('dragstart', () => {
      dragId = Number((t as HTMLElement).dataset.id);
    });
  });
...
searchInput.addEventListener('input', () => {
  ...
  document.querySelectorAll('.task').forEach((task) => {
    ...
    (task as HTMLElement).style.display = txt.includes(keyword) ? '' : 'none';
  });
});
