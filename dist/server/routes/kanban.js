...
router.get('/', async (req, res) => {
    try {
        const r = await db_1.pool.query('SELECT * FROM kanban_cards ORDER BY id');
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const { title, description, status, date_start, date_end, checklist } = req.body;
        const SQL = `
    ...
    `;
        ...
        const r = await db_1.pool.query(SQL, params);
...
router.put('/:id', async (req, res) => {
    try {
      ...
      RETURNING *`;
        const r = await db_1.pool.query(query, [...values, id]);
        res.json(r.rows[0]);
    }
    catch (err) {
...
router.delete('/:id', async (req, res) => {
    try {
        const id = Number(req.params.id);
        await db_1.pool.query('DELETE FROM kanban_cards WHERE id = $1', [id]);
