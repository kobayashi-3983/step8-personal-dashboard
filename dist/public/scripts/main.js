...
// 📅 カレンダープレビュー
// ================================
import { renderCalendarPreview, setupCalendarPreviewWatchers, } from './widgets/calendar-widget.js';
// ================================
// 🪧 他ウィジェット
// ================================
...
function initWidgetSettingsModal() {
    ...
    const toggles = {
        kanban: document.getElementById('set-kanban'),
        weather: document.getElementById('set-weather'),
        calendar: document.getElementById('set-calendar'),
    };
    // 開く
    ...
    save.addEventListener('click', () => {
        Object.entries(toggles).forEach(([key, input]) => {
            localStorage.setItem(`widget-${key}`, input.checked ? 'visible' : 'hidden');
        });
        modal.classList.add('hidden');
        location.reload();
    });
